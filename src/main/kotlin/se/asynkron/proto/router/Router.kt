...
object Router {
    fun newBroadcastGroup (routees : Array<PID>) : Props = Props().withSpawner(spawner(BroadcastGroupRouterConfig(routees)))
    fun newConsistentHashGroup (routees : Array<PID>) : Props = Props().withSpawner(spawner(ConsistentHashGroupRouterConfig(MD5Hasher::hash, 100, routees)))
    fun newConsistentHashGroup (hash : (String) -> Int, replicaCount : Int, routees : Array<PID>) : Props = Props().withSpawner(spawner(ConsistentHashGroupRouterConfig(hash, replicaCount, routees)))
    fun newRandomGroup (routees : Array<PID>) : Props = Props().withSpawner(spawner(RandomGroupRouterConfig(routees)))
    fun newRandomGroup (seed : Int, routees : Array<PID>) : Props = Props().withSpawner(spawner(RandomGroupRouterConfig(seed, routees)))
    fun newRoundRobinGroup (routees : Array<PID>) : Props = Props().withSpawner(spawner(RoundRobinGroupRouterConfig(routees)))
    ...
    fun newRandomPool (props : Props, poolSize : Int, seed : Int) : Props = props.withSpawner(spawner(RandomPoolRouterConfig(poolSize, seed)))
